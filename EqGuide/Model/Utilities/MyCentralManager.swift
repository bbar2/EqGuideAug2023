...
class MyCentralManager: NSObject, CBCentralManagerDelegate {
  ...
  // Once SERVICE found, stop scanning and connect Peripheral
                      ...
                      rssi RSSI: NSNumber)
  {
    ...
    if let discoveredName = discoveredPeripheral.name {
      ...
      if let myPeripheral = nameToMcmDelegate[discoveredName] {
        ...
        print("You might be fighting iOS BLE Name Cache")
        print("Try: Forget all devices from BLE name list.")
        print("     Then cycle power on iOS device.")
      }
